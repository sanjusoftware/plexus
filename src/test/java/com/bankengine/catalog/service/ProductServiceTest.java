...
public class ProductServiceTest extends BaseServiceTest {
    ...
    void testMapPricingFields_ValueExceeds() {
        ...
        PricingComponent pc = new PricingComponent();
        pc.setCode("P1"); pc.setStatus(VersionableEntity.EntityStatus.ACTIVE);
        when(pricingComponentService.getPricingComponentByCode(eq("P1"), any())).thenReturn(pc);
    ...
    void testValidateFeatureValue_BooleanVarious() {
        ...
        FeatureComponent fBool = new FeatureComponent(); fBool.setCode("BOOL"); fBool.setDataType(FeatureComponent.DataType.BOOLEAN); fBool.setStatus(VersionableEntity.EntityStatus.ACTIVE);
        when(featureComponentService.getFeatureComponentByCode("BOOL", null)).thenReturn(fBool);
        when(productRepository.save(any())).thenReturn(product);
        when(productMapper.toResponse(any())).thenReturn(new ProductResponse());
    ...
    void testSyncFeaturesInternal_FatDtoCreation() {
        ...
        FeatureComponentResponse resp = new FeatureComponentResponse(); resp.setId(100L);
        when(featureComponentService.createFeature(any())).thenReturn(resp);
        FeatureComponent comp = new FeatureComponent(); comp.setId(100L); comp.setCode("NEW"); comp.setDataType(FeatureComponent.DataType.STRING); comp.setStatus(VersionableEntity.EntityStatus.ACTIVE);
