...
class FeatureComponentServiceTest extends BaseServiceTest {
    ...
    @Mock private FeatureComponentRepository repository;
    @Mock private ProductFeatureLinkRepository linkRepository;
    @Mock private FeatureComponentMapper mapper;
    ...
    void testGetFeatureResponseById() {
        FeatureComponent f = new FeatureComponent(); f.setBankId(TEST_BANK_ID);
    ...
    void testCreateFeature() {
        FeatureComponentRequest req = new FeatureComponentRequest(); req.setCode("F1");
    ...
    void testVersionFeature_TemporalBranches() {
        ...
        source.setId(1L); source.setBankId(TEST_BANK_ID); source.setCode("F1"); source.setVersion(1); source.setStatus(VersionableEntity.EntityStatus.ACTIVE);
        source.setActivationDate(LocalDate.now()); source.setExpiryDate(LocalDate.now().plusDays(30));

        FeatureComponent newV = new FeatureComponent(); newV.setCode("F1");
    ...
    void testUpdateFeature_UniquenessBranches() {
        FeatureComponent f = new FeatureComponent(); f.setId(1L); f.setBankId(TEST_BANK_ID); f.setCode("OLD"); f.setStatus(VersionableEntity.EntityStatus.DRAFT);
        when(repository.findById(1L)).thenReturn(Optional.of(f));
        when(repository.save(any())).thenReturn(f);
        when(mapper.toResponseDto(any())).thenReturn(new FeatureComponentResponse());

        // Case 1: Same code (skip uniqueness check)
        FeatureComponentRequest req1 = new FeatureComponentRequest(); req1.setCode("OLD");
        service.updateFeature(1L, req1);
        verify(repository, never()).findAllByBankIdAndCode(any(), eq("OLD"));

        // Case 2: New code, not used
        FeatureComponentRequest req2 = new FeatureComponentRequest(); req2.setCode("NEW");
        ...
        FeatureComponent other = new FeatureComponent(); other.setId(2L);
        when(repository.findAllByBankIdAndCode(TEST_BANK_ID, "DUPE")).thenReturn(List.of(other));
        FeatureComponentRequest req3 = new FeatureComponentRequest(); req3.setCode("DUPE");
        assertThrows(IllegalStateException.class, () -> service.updateFeature(1L, req3));
    }

    @Test
    void testActivateFeature_Branches() {
        FeatureComponent f = new FeatureComponent(); f.setId(1L); f.setBankId(TEST_BANK_ID); f.setStatus(VersionableEntity.EntityStatus.DRAFT);
        when(repository.findById(1L)).thenReturn(Optional.of(f));
        when(repository.save(any())).thenReturn(f);
        when(mapper.toResponseDto(any())).thenReturn(new FeatureComponentResponse());
    ...
    void testDeleteFeature_StatusBranches() {
        FeatureComponent f = new FeatureComponent(); f.setId(1L); f.setBankId(TEST_BANK_ID);
        when(repository.findById(1L)).thenReturn(Optional.of(f));
